...
void CAnimationController::SetAllTrackDisable()
{
	for(int i=0;i<m_nAnimationTracks;++i){
		if (m_pAnimationTracks) m_pAnimationTracks[i].SetEnable(false);
	}
}
	...
	if (m_pAnimationTracks)
	{
		...
		if (!m_bAnimationBlending) {
			for (int k = 0; k < m_nAnimationTracks; k++)
			{
				if (m_pAnimationTracks[k].m_bEnable)
				{
					...
					//cout << m_pAnimationTracks[k].m_fPosition << endl;
				}
			}
		}
		else
		{
			...
			for (int j = 0; j < m_pAnimationSets->m_nBoneFrames; j++)
			{
				...
				XMFLOAT4X4 xmf4x4LTrackTransform = pLAnimationSet->GetSRT(j, fLPosition);
				XMFLOAT4X4 xmf4x4UTrackTransform = pUAnimationSet->GetSRT(j, 0);

				XMFLOAT4X4 xmf4x4BTrackTransform = Matrix4x4::Interpolate(xmf4x4UTrackTransform, xmf4x4LTrackTransform, m_fBlendingTime);

				xmf4x4Transform = Matrix4x4::Add(xmf4x4Transform, Matrix4x4::Scale(
					xmf4x4BTrackTransform, 1.0f));
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(1)
{
	...
	pTerrainBaseTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Terrain/Detail_Texture_8.dds", RESOURCE_TEXTURE2D, 0);

	CTexture* pTerrainDetailTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1);
	pTerrainDetailTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Terrain/Detail_Texture_0.dds", RESOURCE_TEXTURE2D, 0);
